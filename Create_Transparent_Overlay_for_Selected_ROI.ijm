  macro "Create overlay for each ROI" {
 /* Peter J. Lee: Applied Superconductivity Center, National High Magnetic Field Laboratory
	v260205: 1st version but based on earlier Create overlay for_each_ROI_v231209-f1.ijm.  v260206: Removed bonus bracket.
	*/
	macroL = "Create_Transparent_Overlay_for_Selected_ROI_v260206.ijm";
	if (Roi.size < 1) exit("Need to select ROI first");
	roiIndex = roiManager("index");
	roiName = RoiManager.getName(roiIndex);
	ascPrefsKey = "asc.ROIColor.Prefs.";
	...
	allColors = Array.concat("none", shadowChoices, colorChoicesStd, colorChoicesMaterials, colorChoicesMod, colorChoicesNeon);
	Dialog.create("Overlay options: " + macroL);
		Dialog.addChoice("Color of ROI overlay outline", allColors, call("ij.Prefs.get", ascPrefsKey + "stroke.color","yellow"));
		Dialog.addNumber("Stroke width width of ROI overlay", parseInt(call("ij.Prefs.get", ascPrefsKey + "stroke.width",1)), 0, 3, "pixels");
		strokeOpacity = parseInt(call("ij.Prefs.get", ascPrefsKey + "stroke.opacity", "50"));
		Dialog.addSlider("Stroke opacity", 0, 100, strokeOpacity);
		shadowOpacity = parseInt(call("ij.Prefs.get", ascPrefsKey + "shadow.opacity", "50"));
		Dialog.addSlider("Shadow opacity \(0=no shadow\)", 0, 100, shadowOpacity);
		Dialog.addNumber("Shadow drop as multiplier of stroke width", parseInt(call("ij.Prefs.get", ascPrefsKey + "shadowdrop.multiplier",1)), 0, 3, "multiplier");
		Dialog.addChoice("Color of ROI overlay fill", allColors, call("ij.Prefs.get", ascPrefsKey + "fill.color", "yellow"));
		fillOpacity = parseInt(call("ij.Prefs.get", ascPrefsKey + "fill.opacity", 0));
	...
	call("ij.Prefs.set", ascPrefsKey + "stroke.width", strokeWidth);
	call("ij.Prefs.set", ascPrefsKey + "stroke.opacity", strokeOpacity);
	call("ij.Prefs.set", ascPrefsKey + "shadow.opacity", shadowOpacity);
	call("ij.Prefs.set", ascPrefsKey + "shadowdrop.multiplier",shadowDropN);
